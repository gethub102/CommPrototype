...
namespace Project4Starter
{
  ...
  public class Receiver
  {
    ...
    public ServiceHost CreateListener() //creating host
    {
    ...
    public bool StartService()
    {
      ...
      try
      {
        host = CreateListener();
        host.Open();
        svc = new CommService(); //get msg to get msg in servers lwb
      }
      catch (Exception ex)
      {
    ...
    public Action defaultServiceAction()
    {
      Action serviceAction = () =>
      {
        ...
        while (true)
        {
          ...
          Console.Write("\n  content is!!! {0}\n", msg.content);
          serverProcessMessage(msg); //when it is default it is empty lwb
    ...
    {
      ThreadStart ts = () => //this thread just start the server act lwb
      {
        if(Util.verbose)
